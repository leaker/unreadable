# Releasing unreadable

How to cut an unreadable release and how the Scoop bucket (`leaker/scoop-bucket`)
...
`GITHUB_TOKEN` is scoped only to `leaker/unreadable`, so a cross-repo push needs
a Personal Access Token stored as a repository secret.
...
6. In `leaker/unreadable`: **Settings → Secrets and variables → Actions → New
...
successful run produces an `unreadable: bump to X.Y.Z` commit in
`leaker/scoop-bucket` (or prints "nothing to push" if already current).
...
`unreadable-0.2.0-rc1-windows-amd64.zip`, but are marked as GitHub
**prereleases** and **skip the Scoop bump** — a pre-release never displaces the
stable version in `scoop update`.

---

## What the release writes to `leaker/scoop-bucket`

`bucket/unreadable.json`, regenerated in full each stable release:
